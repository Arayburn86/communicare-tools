...
.funder-card.amber{border-left-color:#BA7517}.funder-card.red{border-left-color:#C0392B}
...
try:
    from onedrive_helper import (
        load_funders, save_funders,
        DEFAULT_FUNDERS, FUNDER_HEADERS,
    )
...
TIER_META = {
    "A — Top priority":      "#065F46",
    "B — Strong prospect":   "#0F6E56",
    "C — Worth cultivating": "#92400E",
    "Research — Low fit":    "#444441",
}
...
def init_tracker():
    if "funders" not in st.session_state:
        if ONEDRIVE_AVAILABLE:
            with st.spinner("Loading your funders from OneDrive..."):
...
def add_funder(name, url, notes, geo):
    ...
    st.session_state.funders.append({
        ...
        "next_action": "Research giving page", "last_contact_notes": "",
        "grant_range_min": "", "grant_range_max": "",
...
def scrape_funder(name, url):
    result = {"status":"ok","error":"","mission":"","deadline_info":"","contact_info":"",
              "grant_range_min":None,"grant_range_max":None,"raw_text":""}
    try:
...
st.caption("Track foundation prospects — saved permanently to OneDrive.")

if ONEDRIVE_AVAILABLE:
    if st.session_state.last_saved:
        st.markdown(f'<div class="save-ok">✅ Synced to OneDrive at {st.session_state.last_saved}</div>', unsafe_allow_html=True)
else:
    st.markdown('<div class="warn">⚠ OneDrive not configured — data resets when you close the browser.</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with st.form("add_funder"):
        ...
        new_url   = st.text_input("Giving page URL")
        new_geo   = st.selectbox("Geography", ["Rhode Island","New England","National","International"])
        new_notes = st.text_input("Notes")
        if st.form_submit_button("Add"):
            if new_name and new_url:
                add_funder(new_name, new_url, new_notes, new_geo)
                st.success(f"Added: {new_name}" + (" — saved to OneDrive." if ONEDRIVE_AVAILABLE else "."))
                st.rerun()
    st.markdown("---")
    st.markdown("### Filters")
    ...
    for f in filtered:
        ...
        with c2:
            ...
            new_action = st.text_input("Next action", value=f.get("next_action",""),
                key=f"action_{f['name']}", placeholder="Next action...",
...
with tab_pipeline:
    ...
    stages = [
        ...
        ("Relationship","Active cultivation"),
    ...
    for stage, desc in stages:
        ...
        st.markdown(
            f'<div style="background:{color};border-radius:8px;padding:8px 14px;margin-bottom:6px;'
...
with tab_scrape:
    ...
    st.markdown('<div class="hint">Visits each foundation\'s giving page and extracts grant ranges, deadlines, and contact info. Updates are saved to OneDrive.</div>', unsafe_allow_html=True)
    ...
    if unscraped:
        ...
        if st.button("🌐 Scrape selected", use_container_width=False):
...
with tab_export:
    ...
    df = pd.DataFrame([{
        ...
        "Status": f.get("relationship_status",""),
        "Next Action": f.get("next_action",""), "Notes": f.get("notes",""),
        "Deadline": f.get("deadline_info",""), "Matched": f.get("matched_terms",""),
